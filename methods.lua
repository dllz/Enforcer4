...
local function sendRequest(url)
	...
	if code ~= 200 then
		...
		if code ~= 403 and code ~= 429 and code ~= 110 and code ~= 111 and code ~= 116 and code ~= 112 then --error generated by users dumbness that don't need to be logged
...
local function getUpdates(offset)
	--print(clr.reset..clr.red..'['..os.date('%X')..']'..clr.white..' Getting Updates')
...
local function sendMessage(chat_id, text, use_markdown, reply_to_message_id, send_sound)
	...
	if string.len(text) >= 1024 then
		api.sendMessage(chat_id, "Message to long, please decrease the number of characters used")
		return
	end
	...
	if not res and code then --if the request failed and a code is returned (not 403 and 429)
		if code ~= 403 and code ~= 429 and code ~= 110 and code ~= 111 and code ~= 116 and code ~= 131 and code ~= 112 then
			save_log('send_msg', code..'\n'..text)
		elseif code == 112 then
			print("markDown Failed")
			local url = BASE_URL .. '/sendMessage?chat_id=' .. chat_id .. '&text=' .. URL.escape(text)
...
local function sendDocumentWithCapId(chat_id, file_id, caption, reply_to_message_id)
	
	local url = BASE_URL .. '/sendDocument?chat_id=' .. chat_id .. '&document=' .. file_id..'&caption='..caption
...
return {
	...
	sendDocumentWithCapId = sendDocumentWithCapId
}	