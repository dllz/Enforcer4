...
local function sendRequest(url)
	...
	if code ~= 200 then
		...
		if code ~= 429 and code ~= 110 and code ~= 111 and code ~= 116 and code ~= 112 and code ~= 403 then --error generated by users dumbness that don't need to be logged
...
local function getUpdates(offset)
	...
	local url = BASE_URL .. '/getUpdates?timeout=10'
