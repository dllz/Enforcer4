...
local action = function(msg, blocks, ln)
	...
	if blocks[1] == 'editmsg' then
		...
		local msgid = blocks[3]
		local newMsg = blocks[4]
		local res = api.editMessageText(chatid, msgid, newMsg)
		if res then 
			api.sendAdmin("Messaged edited succesfully")
		else 
			api.sendAdmin("edit failed")
		end
	
	elseif blocks[1] == 'admin' then
		local text = ''
		for k,v in pairs(triggers2) do
			text = text..v..'\n'
		end
		api.sendMessage(msg.from.id, text)
	elseif blocks[1] == 'init' then
		db:bgsave()
		bot_init(true)
	...
	if blocks[1] == 'send' then
		...
		local res, code = api.sendMessage(id, text)
		if res then
			api.sendMessage(msg.chat.id, 'Successful delivery')
		end
		if not res then
			api.sendMessage(msg.chat.id, dump(code)..', '..dump(res))
		end 
	end
